...
***Description: Replication of Table A2
***Balancing Tests for Applicant Firms By Treatment Year
...
if "`dep_var'"=="treatment_full" & `i'==1 local command replace
if "`dep_var'"!="treatment_full" | `i'!=1 local command append

if `i'==1 local years "1940-1942"
if `i'==2 local years "1943"
if `i'==3 local years "1944"
if `i'==4 local years "1945"


reghdfe `dep_var' ${log_ind_var} if alternative_treatment_year==`i', absorb(app_window app_day county_sector) cluster(cluster) `version'
...
outreg2 using "${directory_results}/table_a2a`1'.xls", dec(3) addstat(Mean dep var, `mean3', SD dep var, `sd3', P-value of F-test , `p_value') addtext(Sector FE, NO, App Window FE, YES, App date FE, YES, County FE, NO, Subd FE, NO, TWI FE, NO, County-Sector FE, YES, TWI-Sector FE, NO, Sub-sector FE, NO, Cluster, Sub-App, Years, `years') `command'
...
reghdfe `dep_var' ${log_ind_var} ln_number_contracts ln_value_contracts if alternative_treatment_year==`i', absorb(app_window app_day county_sector) cluster(cluster) `version'
...
outreg2 using "${directory_results}/table_a2b`1'.xls", dec(3) addstat(Mean dep var, `mean3', SD dep var, `sd3', P-value of F-test , `p_value') addtext(Sector FE, NO, App Window FE, YES, App date FE, YES, County FE, NO, Subd FE, NO, TWI FE, NO, County-Sector FE, YES, TWI-Sector FE, NO, Sub-sector FE, NO, Cluster, Sub-App, Years, `years') `command'
...
if "`dep_var'"=="treatment_full" & "`year'"=="1920" & `i'==1 local command replace
if "`dep_var'"!="treatment_full" | "`year'"!="1920" | `i'!=1 local command append
...
reghdfe `dep_var' ${var_manuf_`year'} ${var_demo_`year'} if alternative_treatment_year==`i', absorb(subdistrict_sector app_window app_day) cluster(cluster) `version'
...
outreg2 using "${directory_results}/table_a2c`1'.xls", dec(3) addstat(Mean dep var, `mean3', SD dep var, `sd3', P-value of F-test , `p_value')  addtext(App Window FE, YES, App date FE, YES, County FE, NO, Subd FE, NO, TWI FE, NO, TWI-Sector FE, NO, Sub-Sector FE, YES, Cluster, Sub-App, Years, `years') `command'
