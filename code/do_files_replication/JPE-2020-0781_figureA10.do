...
***Description: Replication of Figure A10
...
reghdfe `dep_var' ${year_effects} if balanced==1 & second_treatment=="" & median_draftees==`i', absorb(app_window app_day county_sector_time) cluster(cluster) `version'
preserve
regsave, ci
...
(rcap ci_upper ci_lower var if treatment==1 & alt_treat==3, color("0 63 92") lwidth(vthin)) (connected coef var if treatment==1 & alt_treat==3, color("0 63 92%30") msymbol(Oh) mlwidth(thin) msize(large) lwidth(vthin) lpatter("--..."))  ///
, yline(0, lcolor(black)) xline(-0.8, lcolor(maroon*0.3)) xline(3.2, lcolor(maroon*0.3)) xlabel(-5(1)10) ylabel(-0.1(0.05)0.30, format(%6.2f) nogrid angle(0)) xtitle(Years after training) legend(ring(0) pos(11) col(1) size(vsmall) order(2 4 6) label(2 "Tertile 1") label(4 "Tertile 2") label(6 "Tertile 3")) graphregion(color(white))
graph export "${directory_results}/figureA10_`dep_var'_draftees`1'.pdf", replace
...
reghdfe `dep_var' ${year_effects} if balanced==1 & second_treatment=="" & median_managers==`i', absorb(app_window app_day county_sector_time) cluster(cluster) `version'
preserve
regsave, ci
...
(rcap ci_upper ci_lower var if treatment==1 & alt_treat==3, color("0 63 92") lwidth(vthin)) (connected coef var if treatment==1 & alt_treat==3, color("0 63 92%30") msymbol(Oh) mlwidth(thin) msize(large) lwidth(vthin) lpatter("--..."))  ///
, yline(0, lcolor(black)) xline(-0.8, lcolor(maroon*0.3)) xline(3.2, lcolor(maroon*0.3)) xlabel(-5(1)10) ylabel(-0.1(0.05)0.30, format(%6.2f) nogrid angle(0)) xtitle(Years after training) legend(ring(0) pos(11) col(1) size(vsmall) order(2 4 6) label(2 "Tertile 1") label(4 "Tertile 2") label(6 "Tertile 3")) graphregion(color(white))
graph export "${directory_results}/figureA10_`dep_var'_managers`1'.pdf", replace
